...
Return the compact findings digest required by the system prompt. Do not call any report tool. Do not include executive summary, stack recap, priority list, code snippets, or follow-up offers. If you are near the tool limit, stop using tools and synthesize from available evidence.`,
        ...
        console.log(
            `[contentAgent] Complete. Findings length: ${rawFindings.length} chars, ${totalToolCalls} tool calls`
        );
        console.log(`[contentAgent] Execution time: ${executionTimeMs}ms`);
        ...
        const logFileName = `content-agent-${repositoryId}-${Date.now()}.json`;
        ...
        const logFileName = `content-agent-ERROR-${repositoryId}-${Date.now()}.json`;
        const logPath = path.join(logDir, logFileName);
        fs.writeFileSync(logPath, JSON.stringify(agentLog, null, 2));
        console.error(`[contentAgent] Error log written to: ${logPath}`);
