    ...
    try {
        ...
        const stream = new ReadableStream({
            async start(controller) {
                ...
                try {
                    const output = await runTransactionAgent({ repositoryId: String(repositoryId), installationId: String(installationId), onEvent: send });
                    ...
                    console.error("[api/agent/transaction-test] Stream error:", message);
        ...
        const message = error instanceof Error ? error.message : "Unknown error";
        console.error("[api/agent/transaction-test] Error:", message);
