...
import { runComputeHeavyAgent, StreamEvent } from "../../../../../actions/agents/compute-heavy";
import { getInstallationToken } from "@/lib/github";

/**
 * POST /api/agent/compute-heavy-test
 * Runs the Compute-heavy Agent for a given repository.
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!resolvedToken && installationId) {
            console.log("[api/agent/compute-heavy-test] No access token; generating installation token for", installationId);
        ...
        const stream = new ReadableStream({
            async start(controller) {
                ...
                try {
                    const output = await runComputeHeavyAgent({
                    ...
                    console.error("[api/agent/compute-heavy-test] Stream error:", message);
                    try {
                        controller.enqueue(
        ...
        const message = error instanceof Error ? error.message : "Unknown error";
        console.error("[api/agent/compute-heavy-test] Error:", message);
        return new Response(
